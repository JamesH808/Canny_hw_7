...
/* CENG 381 Assignment 7 solution*/
	   ...
	   for(i=0; i<IMG_NUM; i++)
	   {
	      ...
	      ****************************************************************************/
	      ...
	      sc_time current_time = sc_time_stamp();
              tim_out.write(current_time);
              printf("%.2f  s: Stimulus sent frame %d \n", current_time.to_seconds(), i);
	   }
	}
...
SC_MODULE(Monitor){
	...
	******************************************************************************/
	...
	void main(void)
	{
	   ...
	   {
	      ImgIn.read(imagein);
	      tim_in.read(sent_time);
	      sc_time delay;
	      sc_time current_time = sc_time_stamp();
	      delay = current_time - sent_time;
	      sc_time difference = current_time - last_time;
	      double timeDifferenceInSeconds = difference.to_seconds();
	      double fps = 1/(timeDifferenceInSeconds);
	      printf("%.2f  s: Monitor received frame %d with %.2f  s delay.\n", current_time.to_seconds(), i+1, delay.to_seconds());
	      printf("%.2f s: %.3f seconds after previous frame, %.3f FPS\n", current_time.to_seconds(),difference.to_seconds(),fps);
